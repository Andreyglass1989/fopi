...
API = '5576706434:AAHFT78UcKFHXW3F2XBjvg76TOZStIYzXCw'
# 5576706434:AAHFT78UcKFHXW3F2XBjvg76TOZStIYzXCw
...
def some_func(call, page=1):
    ...
    if call.data.isdigit() and call.data not in list_menu:
        ...
        bot.send_message(call.message.chat.id, string_all, parse_mode='HTML')
...
def send_order_page(message, page=1, order_count=0, crw=None):
    ...
    for crw1 in crw:
        ...
        if crw1.currency_id == 2:
            ...
            string_1 = "🗓" + crw1.date_added.strftime("%d.%m.%y") +  "🧾№" "<a href='#'>" + str(crw1.order_id) + "</a> - сума - %s %s" %(total, crw1.currency_code)+ "\n"
        else:
    ...
    string_all = string_all + "\n\nДля того, щоб переглянути чек клікніть по номеру замовлення біля 🧾"
